...
Search by `feature_id` or title match.

**If NOT found:** POST to create a new record. Assign `activity_number` (query highest existing + 1).

**If found — collision check:**
Before updating, check if any channel content fields you want to write are already non-empty.
- If no collisions (only filling empty slots): PUT to update. No user intervention needed.
- If collisions detected: Ask the user via AskUserQuestion with options:
  - "Replace existing" (overwrite + log override in edit_history)
  - "Create new activity (v2)" (POST new record, same feature_id)
  - "Cancel"

See push-to-activity skill Step 4 for the full collision detection flow.
...
```json
{
  ...
  "created_by_user": "{CURRENT_USER_NAME_OR_EMAIL}",
  "created_by_tool": "Marketing Plugin v1.18",
  "generation_method": "ai_generated",
  "pushed_at": "{CURRENT_ISO_TIMESTAMP}",
  "content_maturity": "raw_draft",
  "last_edited_by": "{CURRENT_USER_NAME_OR_EMAIL}",
  "last_edited_at": "{CURRENT_ISO_TIMESTAMP}",
  "edit_history": [{
    "timestamp": "{CURRENT_ISO_TIMESTAMP}",
    "user": "{CURRENT_USER_NAME_OR_EMAIL}",
    "action": "pushed_content",
    "tool": "Marketing Plugin v1.18",
    "channels_affected": ["{LIST_OF_CHANNELS_WITH_CONTENT}"],
    "note": "AI-generated content from feature-brief (#feat-[name])"
  }],
...
**IMPORTANT:** Always `status: "new"` and `approval_status: "draft"` to match the team's pattern.
